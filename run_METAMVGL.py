...
parser.add_argument(
    ...
    required=False,
    default=None,
    help="the result dir of arbitrary initial binning tool (-b is ignored if this parameter is set)",
)
parser.add_argument(
    "-r1",
    "--reads1",
...
if args.initial_binning.lower() not in ("maxbin2", "metabat2"):
    logging(
        "Please check the --initial_binning or -b, which should be MetaBat2 or MaxBin2."
    )
    sys.exit(1)

if args.initial_binning_result is not None and not os.path.isdir(args.initial_binning_result):
    logging(
        "Please check the --initial_binning_result or -br, which should be an output directory of a initial binning tool."
    )
    sys.exit(1)
...
bindir = ""
bamfile = ""
if args.initial_binning_result is None:
    ...
    bindir = args.initial_binning_result
